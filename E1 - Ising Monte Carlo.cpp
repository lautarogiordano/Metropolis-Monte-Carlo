...
void system::update(int plot, int midvalues){			//Funcion que actualiza el sistema. Cada vez que la llamo se cumple un paso de montecarlo
	...
	for(int i=0;i<L;i++){								//Recorro SECUENCIALMENTE la red. Esto no es puramente random pero agiliza la ejecucion un monton. 
		for(int j=0;j<L;j++){							//Seleccionar el indice al azar no tiene mucha incidencia en el resultado de la simulacion
...
int main()
{
	...
	while(retry){
		printf("\n--------------------------------------------------------------------------------\n");
		printf("Numero de ejercicio (a=1 b=2 ...):  ");
		scanf("%d",&run);
